export default function Track({ number, trackData, onUnassignPlatform }) {
  if (!trackData) {
    return (
      ...
      {isOccupied && trainDetails ? (
        <div>
          <div className="flex justify-between items-center mt-1">
            ...
            <button onClick={() => onUnassignPlatform(trackData.id, trainDetails)} className="bg-red-500 text-white text-xs font-bold py-1 px-3 rounded-md hover:bg-red-600 transition-all flex-shrink-0">Unassign</button>
          </div>
          {actualArrival && (
            <p className={`text-xs mt-1 font-semibold ${isOccupied ? 'text-green-200' : 'text-gray-600'}`}>Actual Arrival: {actualArrival}</p>
          )}
        </div>
